...
| `setDiffModels`           | `(models: DiffModelPair, options?: DiffModelTransitionOptions) => Promise<void>`                                      | Swap changed diff models in place; same-content calls keep the visible models and refresh language/presentation to avoid a transient blank frame |
