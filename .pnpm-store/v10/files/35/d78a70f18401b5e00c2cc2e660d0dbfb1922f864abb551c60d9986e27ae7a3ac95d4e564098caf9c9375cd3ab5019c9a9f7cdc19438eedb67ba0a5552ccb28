...
function parseMergeConflictDiffFromFile(file, maxContextLines = 6) {
	...
	if (s.hunks.length > 0 && s.additionLines.length > 0 && s.deletionLines.length > 0) {
		...
		const collapsedAfter = Math.max(s.additionLines.length - getHunkSideEndBoundary(lastHunk.additionStart, lastHunk.additionCount), 0);
...
function finalizeActiveHunk(s) {
	...
	const additionStart = hunk.additionCount === 0 ? hunk.additionStart - 1 : hunk.additionStart;
	const deletionStart = hunk.deletionCount === 0 ? hunk.deletionStart - 1 : hunk.deletionStart;
	const collapsedBefore = Math.max(getHunkSideStartBoundary(additionStart, hunk.additionCount) - s.lastHunkEnd, 0);
	const finalizedHunk = {
		collapsedBefore,
		additionStart,
