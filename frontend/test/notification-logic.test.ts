...
describe('shouldNotifyCompletion', () => {
  it('returns true only for idle + no pending question', () => {
    expect(shouldNotifyCompletion('idle', false)).toBe(true);
  });

  it('returns false for aborted', () => {
    expect(shouldNotifyCompletion('aborted', false)).toBe(false);
  });

  it('returns false when pending question exists', () => {
    expect(shouldNotifyCompletion('idle', true)).toBe(false);
  });
});
...
describe('notification tags', () => {
  ...
  const { maybeNotifyCompletion, maybeNotifyQuestion } = useNotification();
  ...
  it('question tags are per-request', () => {
