...
<!-- pending question cards and the composer. Only rendered inside a -->
<!-- chat-pane group so it never leaks into files/tasks/preview/btw panes. -->
<script setup lang="ts">
import { onMounted, onUnmounted, ref, watch } from 'vue';
import { useI18n } from 'vue-i18n';
import type { ActivationBadges, ConversationStatus, QueuedPromptView, TaskItem, TodoView, UIQuestion } from '../../types';
...
function loadForEdit(value: string): boolean {
  ...
  // question is shown it is unmounted, so report unavailability so
