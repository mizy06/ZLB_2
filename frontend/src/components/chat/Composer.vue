...
const props = withDefaults(defineProps<{
  ...
  /** Status data (model and context) — drives the bottom toolbar. */
...
function selectDrawingMode(multiAgent: boolean): void {
  if (multiAgent !== multiAgentOn.value) emit('toggleSwarm');
  closeModes();
}

// Drawing-mode selector: single-editor direct generation or a fixed two-round
// multi-role review/edit loop.
...
const MODE_DESC_KEYS = ['status.singleAgentDrawingDesc', 'status.multiAgentDrawingDesc'] as const;
    ...
    <div class="composer-card">
      ...
      <div class="toolbar">
        ...
        <div class="toolbar-left">
          ...
          <div v-if="status" ref="modesRef" class="modes">
            <button
              type="button"
              class="mode-pill"
              :class="{ on: multiAgentOn, open: modesOpen }"
              @click.stop="toggleModes"
            >
              <span class="mode-label">{{ selectedDrawingModeLabel }}</span>
            ...
            <div v-if="modesOpen" ref="modesMenuRef" class="modes-menu" :style="modesMenuInlineStyle" role="menu">
              <button
                type="button"
                class="mode-row"
                :class="{ on: !multiAgentOn }"
                role="menuitemradio"
                :aria-checked="!multiAgentOn"
                @click="selectDrawingMode(false)"
              >
                <span class="mode-row-icon"><Icon name="file-edit" size="sm" /></span>
                <span class="mode-row-info">
                  <span class="mode-row-name">{{ t('status.singleAgentDrawingLabel') }}</span>
                  <span class="mode-row-desc">{{ t('status.singleAgentDrawingDesc') }}</span>
                </span>
                <span class="mode-check"><Icon v-if="!multiAgentOn" name="check" size="sm" /></span>
              </button>
              <button
                type="button"
                class="mode-row"
                ...
                role="menuitemradio"
                :aria-checked="multiAgentOn"
                @click="selectDrawingMode(true)"
              >
                <span class="mode-row-icon"><Icon name="sparkles" size="sm" /></span>
                <span class="mode-row-info">
                  <span class="mode-row-name">{{ t('status.multiAgentDrawingLabel') }}</span>
                  <span class="mode-row-desc">{{ t('status.multiAgentDrawingDesc') }}</span>
                </span>
                <span class="mode-check"><Icon v-if="multiAgentOn" name="check" size="sm" /></span>
              </button>
            </div>
        ...
        <div class="toolbar-right">
          ...
          <span
            v-if="status"
            class="model-pill model-pill-fixed"
          >
            <b>{{ FIXED_MODEL_LABEL }}</b>
          </span>
...
.mode-row {
  display: grid;
  grid-template-columns: 14px var(--composer-menu-desc-width, max-content) 18px;
  column-gap: 7px;
  row-gap: 2px;
  align-items: start;
  width: 100%;
...
/* ---- Narrow composer toolbar ----------------------------------------------
   ...
   signal; the exact numbers live in its tooltip), and the model name truncates
   earlier so the ring and send button are never squeezed out. */
...
@media (max-width: 640px) {
  ...
  /* Mobile toolbar: hide secondary controls; attach / context ring / model /
     send stay visible. Agent modes move into the MobileSettingsSheet.
