...
<!-- controls as big tappable rows — fixed model, thinking level, drawing mode, and a -->
...
const drawingMode = computed(() => swarmOn.value ? 'multi' : 'single');
const drawingModeDescription = computed(() =>
  t(swarmOn.value ? 'status.multiAgentDrawingDesc' : 'status.singleAgentDrawingDesc'),
);
...
function setDrawingMode(value: string): void {
  const multiAgent = value === 'multi';
  if (multiAgent !== swarmOn.value) emit('toggleSwarm');
}
...
<template>
  ...
  >
    ...
    <div class="srow read-only mode-setting">
      <span class="srow-main">
        <span class="srow-label">{{ t('status.modesLabel') }}</span>
        <span class="srow-sub">{{ drawingModeDescription }}</span>
      </span>
      <SegmentedControl
        :model-value="drawingMode"
        :options="[
          { value: 'single', label: t('status.singleAgentDrawingLabel') },
          { value: 'multi', label: t('status.multiAgentDrawingLabel') },
        ]"
        size="sm"
        @update:model-value="setDrawingMode"
      />
    </div>
