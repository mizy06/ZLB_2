...
<template>
  <div class="se" :class="{ on: active }" @click="emit('select', session.id)">
    <div class="row">
      ...
      <!-- Pending question tag, shown even when the row isn't active. -->
      ...
      <Tooltip :text="t('workspace.abortedTitle')">
        <Badge
          v-if="!renaming && !session.busy && session.pendingInteraction !== 'question' && questionCount === 0 && (session.lastTurnReason === 'cancelled' || session.lastTurnReason === 'failed')"
          variant="danger"
          size="sm"
        >
          {{ t('workspace.aborted') }}
