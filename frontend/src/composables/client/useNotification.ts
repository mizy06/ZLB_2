...
// a question waiting for an answer. Each kind has its own on/off preference
// (persisted) plus the shared OS permission + Notification
...
//
// Completion notifications default on, while question notifications surface
// request text and default off.
...
export function shouldNotifyCompletion(
  ...
  return status === 'idle' && !hasPendingQuestion;
}
