...
> **2026-07-29 架构废止说明：** 本文保留为 C+ 历史设计记录。第 4.4 节的
> “双向建图”、第 11.4 节允许底层节点簇补建父节点的结构权限，已被
> [思维导图系统级整改方案](../../MINDMAP_SYSTEM_REDESIGN.md) 中的 ADR-01
> 替代。当前决策是“结构只向下生长，证据从下向上验收”：只有 Global /
> Recursive Region Planner 可以生成结构；bottom-up Agent 只能审计并提交
> `ReplanRequest`。
>
> 完整替代 Workflow 见
> [思维导图 vNext 端到端 Workflow 规范](../../MINDMAP_WORKFLOW_SPEC.md)。
> 该规范当前状态为 `MIXED`：ADR-01 已于 2026-07-29 批准；ADR-02
> “合同基础与受限 no-egress shadow”已于 2026-07-30 批准，但尚未收到明确
> 启动编码指令；ADR-03 至 ADR-12 已于 2026-07-30 完成分轴产品审批，精确范围见
> [产品审批记录](../../MINDMAP_PRODUCT_APPROVAL_RECORD.md)。该审批未恢复本文的
> 双向结构权限，也不授权公网发布。
...
5. ~~两路结果在中层对齐，允许重命名、拆分、删除或补建抽象父节点。~~
   **已废止：** bottom-up 只能提交 `ReplanRequest`，不能补建结构父节点。
...
### 4.4 双向建图（历史方案，已被 ADR-01 废止）

> 本节只保留历史背景，不得作为当前实现依据。当前结构生成只允许自顶向下
> RegionPlan 递归；自底向上只做证据和遗漏审计。
...
### 11.4 双向对齐（历史方案，结构补建权限已废止）

> 以下内容中的“底层补建父节点”已禁止。结构不匹配只能生成
> `ReplanRequest`，并由受影响的最小祖先 Region Planner 重规划。

- 顶层 Skeleton 必须被底层节点簇支持。
- ~~底层节点簇可补建顶层没有提出的父节点。~~ **已废止。**
- 不匹配的一级主题可以重命名、拆分或删除。
- 不为了视觉对称而发明层级。
